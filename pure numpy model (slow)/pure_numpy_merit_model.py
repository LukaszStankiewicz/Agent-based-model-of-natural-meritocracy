...
class Simulator_simple_git:
    ...
    def setup_merit_agents(self):
        ...
        agents[:, 3] = np.zeros(self.seed_population)  # UPDATED
        agents[:, 4] = np.zeros(self.seed_population)  # UPDATED
    ...
    def real_scale_plot(self, mode="auto", agents="", scale=1000, show_colors=False):
        """
        This method is not used during the simulation but it can be called to generate a graphics of agents in real scale.

        :param mode                   (string) : set to "auto" to let the method autmatically draw data from class variables
                                                 set to "manual" to provide data as arguments.
        :param agents            (numpy array) : An array with agent data from a single simulation run.
        :param scale                     (int) : Scale parameter defaults to 1000 and should be the same as plane_size used
                                                 for the simulation.
        ...
        # Set ax limits and aspect ratio.
    ...
    def update_plot(self, i):
        ...
        # Updates agents.
        self.agents = self.update_agents(i)
    ...
    def update_agents(self, frame):
        ...
        for x in range(self.iterations_per_frame):

            # Updates contact_range if contact range is set to diminish each time step (by a contact_range_falloff being more than 0).
            if self.agent_contact_range > 0:
                if self.contact_range_falloff > 0:
                    self.agent_contact_range = self.agent_contact_range - (
                        x * 0.01 * self.contact_range_falloff * self.agent_contact_range
                    )
            ...
            near_arr_xy = np.zeros((len(self.agents), len(self.agents)))

            # Create array for listing agents that are close to walls.
            wall_arr = np.zeros((len(self.agents), 4))

            # Finds active agents in range of other agents, and agents that are near to walls.
            for z in range(len(self.agents)):

                # Finds agents that are near other agents.
                distance = np.sqrt(
                    (np.abs(self.agents[:, 1] - self.agents[z, 1])) ** 2
                    + (np.abs(self.agents[:, 2] - self.agents[z, 2])) ** 2
                ) < (
                    self.agent_contact_range
                    + ((self.agents[:, 5] + self.agents[z, 5]) / self.plane_size)
                )
                near_arr_xy[z] = distance
                near_arr_xy[z, z] = False

                # Find agents that are near walls.
                wall_arr[z, 0] = self.agents[z, 1] < (
                    self.agent_contact_range + self.agents[z, 5] / self.plane_size
                )
                wall_arr[z, 1] = 1 - self.agents[z, 1] < (
                    self.agent_contact_range + self.agents[z, 5] / self.plane_size
                )
                wall_arr[z, 2] = self.agents[z, 2] < (
                    self.agent_contact_range + self.agents[z, 5] / self.plane_size
                )
                wall_arr[z, 3] = 1 - self.agents[z, 2] < (
                    self.agent_contact_range + self.agents[z, 5] / self.plane_size
                )
                ...
                if self.agents[y, 8] == 1:
                    ...
                    if self.agents[y, 8] == 1:
                        ...
                        if self.model_type == 0:
                            self.agents[y, 7] += self.agents[y, 6] * 2

                        # Production function for exponential model of production.
                        elif self.model_type == 1:
                            self.agents[y, 7] *= 1 + (self.agents[y, 6] / 100)

                        # Updates agent's size and total production time parameters.
                        self.agents[y, 5] = np.sqrt(self.agents[y, 7] / 3.14)
                        self.agents[y, 9] += 1

            # Moves agents that had their momentum parameter set above 0.
            self.agents[:, 1] = self.agents[:, 1] + (
                self.agents[:, 3] * self.agent_speed
            )
            self.agents[:, 2] = self.agents[:, 2] + (
                self.agents[:, 4] * self.agent_speed
            )

            # Keeps agents in the plane.
            self.agents[:, 1][self.agents[:, 1] < 0] = 0
            self.agents[:, 2][self.agents[:, 2] < 0] = 0
            self.agents[:, 1][self.agents[:, 1] > 1] = 1
            self.agents[:, 2][self.agents[:, 2] > 1] = 1

            # If all agents are in state 2 (end state) it ends the model run.
            if np.all(self.agents[:, 8] == 2):
                return self.agents
